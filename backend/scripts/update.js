...
const createBackupTables = async () => {
  console.log('UPDATE: Creating backup of cards and card_pack_appearances tables...');

  try {
    // Drop old backup tables if they exist
    console.log('UPDATE: Removing old backup tables if they exist...');
    await query('DROP TABLE IF EXISTS cards_backup CASCADE;');
    await query('DROP TABLE IF EXISTS card_pack_appearances_backup CASCADE;');

    // Create backup of cards table
    console.log('UPDATE: Creating backup of cards table...');
    await query(`
      CREATE TABLE cards_backup AS
      SELECT * FROM cards;
    `);

    // Create backup of card_pack_appearances table
    console.log('UPDATE: Creating backup of card_pack_appearances table...');
    await query(`
      CREATE TABLE card_pack_appearances_backup AS
      SELECT * FROM card_pack_appearances;
    `);

    // Get count of backed up records for verification
    const cardsBackupResult = await query('SELECT COUNT(*) FROM cards_backup;');
    const appearancesBackupResult = await query('SELECT COUNT(*) FROM card_pack_appearances_backup;');

    console.log(`UPDATE: Backup complete!`);
    console.log(`  - Backed up ${cardsBackupResult.rows[0].count} cards`);
    console.log(`  - Backed up ${appearancesBackupResult.rows[0].count} card pack appearances`);

  } catch (err) {
    console.error('UPDATE: Error creating backup tables:', err);
    throw new Error(`Failed to create backup tables: ${err.message}`);
  }
};
...
const handleCardWithReprintLogic = async (cardData, packCode) => {
  ...
  // Check card id (not card code) for reprint pattern
  if (isReprint(cardId)) {
...
const main = async () => {
  ...
  try {
    ...
    // Create backup tables before starting the update process
        ...
        for (const cardData of cardsFromPack) {
          try {
            if (!cardData || typeof cardData !== 'object' || !cardData.card_id || !cardData.name) {
              console.warn(`UPDATE: Skipping card in pack ${pack.code}:`, cardData);
              continue;
            }
