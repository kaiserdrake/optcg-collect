...
const CardVariantIndicator = ({ cardId }) => {
    // Add null/undefined check before calling .match()
    if (!cardId || typeof cardId !== 'string') {
        return null; // Return null if cardId is undefined, null, or not a string
    }
    ...
    const label = `Alternate Art #${number} - ${cardId}`;
