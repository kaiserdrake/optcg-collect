...
const LocationManagementModal = ({ isOpen, onClose, onLocationChange }) => {
  ...
  const validateForm = (location) => {
    ...
    if (!location.type) {
      dispatch({ type: 'SET_ERROR', field: 'type', message: 'Type is required.' });
      isValid = false;
    }
  ...
  const handleUpdateLocation = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Trigger refresh callback for parent components
        if (onLocationChange) {
          onLocationChange('update', data);
        }
      } else {
        if (res.status === 409) {
          dispatch({ type: 'SET_ERROR', field: 'name', message: data.message });
  ...
  const handleDeleteLocation = async () => {
    const locationToDelete = state.locationToDelete;

    dispatch({ type: 'SET_LOADING', payload: true });
    try {
      const res = await fetch(`${api}/api/locations/${locationToDelete.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const data = await res.json();
      ...
      if (res.ok) {
        ...
        // Trigger refresh callback for parent components
        if (onLocationChange) {
          onLocationChange('delete', locationToDelete);
        }
      } else {
        toast({
          title: 'Error',
